...
// define an http agent which keeps the connections alive per aws best practice
 ...
 *   - https://docs.aws.amazon.com/sdk-for-javascript/v3/developer-guide/node-reusing-connections.html
 *   - > By default, the default Node.js HTTP/HTTPS agent creates a new TCP connection for every new request. To avoid the cost of establishing a new connection, you can reuse an extant connection.
 ...
 */
export const getDocumentClient = (context: {
  aws: { dynamodb: { sdk: SdkAwsDynamodb } };
}): DynamoDBDocumentClient => {
  if (cachedDocumentClient) return cachedDocumentClient;

  // destructure SDK from context
  const { DynamoDBClient, DynamoDBDocumentClient, NodeHttpHandler } =
    context.aws.dynamodb.sdk;

  // create the base client with connection reuse
